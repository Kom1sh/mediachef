...
const KNOWN_NETS = {
  ...
  DuckDuckBot: [8075],
  // Вторая сеть — 207304 (Казахстан). Сначала журнал пометил её подделкой, но
  // поведение говорит обратное: 7 сентября робот из неё запросил два случайных
  // несуществующих адреса, получил 404, и через час в Вебмастере обновилась
  // проверка «мягкий 404». При этом он же забирает sitemap.xml, llms.txt и
  // страницы всех языков, а из 13238 в те же дни шёл только /robots.txt.
  YandexBot: [13238, 207304],
  Applebot: [714],
