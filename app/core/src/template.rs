            ...
            ParamType::Enum => {
                ...
                if !vals.contains(&val) {
                    return Err(TemplateError::InvalidParam {
                        key: p.key.clone(),
                        reason: format!("{val} not in {vals:?}"),
                    });
                }
            }
            ParamType::Int | ParamType::Float if !val.is_empty() => {
                let n: f64 = val.parse().map_err(|_| TemplateError::InvalidParam {
                    key: p.key.clone(),
...
fn substitute(
    ...
    while let Some(start) = rest.find('{') {
        let end = rest[start..]
            .find('}')
            .map(|e| start + e)
            .ok_or_else(|| TemplateError::UnknownPlaceholder(rest.to_string()))?;
        out.push_str(&rest[..start]);
        let name = &rest[start + 1..end];
        let val = lookup(name, input, output, params)
...
pub fn build_argv(
    ...
    for token in &recipe.args {
        ...
        if token.starts_with('{') && token.ends_with(":split}") {
            ...
            let raw = lookup(name, input, output, params)
                .ok_or_else(|| TemplateError::UnknownPlaceholder(name.to_string()))?;
            let pieces = shell_words::split(&raw).map_err(|e| TemplateError::InvalidParam {
                key: name.to_string(),
                reason: e.to_string(),
            })?;
...
"#,
    ...
    const START: &str =
        r#"  - {key: start, type: string, default: "", label: {en: S, ru: С}, advanced: true}"#;
    ...
    fn substitutes_tokens_and_substrings() {
        ...
        let v = build_argv(
            ...
            "/in путь/a b.mp4",
            "/out/o.mp4",
            &resolve_params(&r, &HashMap::new()).unwrap(),
        )
        .unwrap();
        assert_eq!(
            v,
    ...
    fn value_stays_single_token_no_injection() {
        let r = recipe(
            &["-i", "{input}", "-metadata", "title={start}", "{output}"],
            START,
        );
    ...
    fn empty_split_drops_preceding_flag() {
        const FILTERS: &str =
            r#"  - {key: filters, type: string, default: "", label: {en: F, ru: Ф}}"#;
        let r = recipe(
            &["-i", "{input}", "-vf", "{filters:split}", "{output}"],
            FILTERS,
        );
        let v = build_argv(
            &r,
