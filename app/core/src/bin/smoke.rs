...
fn main() {
    ...
    for r in catalog::bundled() {
        if !matches!(r.engine, Engine::Ffmpeg) { continue; }
        ...
        if !staged.exists() { std::fs::copy(&input, &staged).unwrap(); }
        let output = naming::plan_output(&r, &staged);
        let params = template::resolve_params(&r, &HashMap::new()).unwrap();
        let argv = template::build_argv(&r, &staged.display().to_string(), &output.display().to_string(), &params).unwrap();
        let res = runner::run_ffmpeg(&ffmpeg, &argv, Some(2.0), &runner::CancelToken::new(), |_| {});
        let ok = res.is_ok()
            && output.exists()
            && probe::probe(&ffprobe, &output).is_ok();
        println!("{} {:<24} -> {}", if ok { "PASS" } else { "FAIL" }, r.id, output.file_name().unwrap().to_string_lossy());
        if !ok {
            failed += 1;
            if let Err(e) = res { eprintln!("  {}\n  {}", e.message, e.stderr_tail); }
        }
    }
    if failed > 0 {
