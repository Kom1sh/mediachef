...
pub struct Dictation {
    ...
    /// можно пользоваться» и «нельзя». `"paste"` кладёт в буфер и нажимает
    /// Cmd+V: быстрее на длинном тексте, но затирает буфер. Оба на macOS
    /// требуют разрешения «Универсальный доступ». Разрешение проверяется
...
fn sanitize_dictation(mut d: Dictation) -> Dictation {
    ...
    // Список закрытый: значение из головы не должно молча включать то, чего
    // нет. «paste» проверяется в момент вставки — если «Универсальный доступ»
    // не выдан, текст всё равно ляжет в буфер, а человеку скажут почему.
    d.delivery = one_of(d.delivery, &["clipboard", "type", "paste"], "clipboard");
...
mod tests {
    ...
    /// Автовставка — допустимое значение: разрешение проверяется в момент
    /// вставки, а не отсекается здесь.
    #[test]
    fn paste_delivery_is_accepted() {
        let mut s = AppSettings::default();
        s.dictation.delivery = "paste".into();
        let s = sanitize(s);
        assert_eq!(s.dictation.delivery, "paste");
    }
