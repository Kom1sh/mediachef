...
pub struct Dictation {
    ...
    /// Имя устройства ввода; пустая строка — вход по умолчанию системы.
    ///
    /// Появилось после живого случая: AirPods в кейсе остаются «подключёнными»
    /// и входом по умолчанию, и macOS отдаёт с них чистые нули — любому
    /// приложению. Явно выбранный микрофон от этого застрахован. Если
    /// названного устройства нет, запись идёт с умолчания.
...
fn sanitize_dictation(mut d: Dictation) -> Dictation {
    ...
    // Имя устройства — любое, белого списка нет: устройства у всех свои.
    // Режем только явный мусор; отсутствие устройства с таким именем
    // обрабатывает запись — берёт вход по умолчанию.
    d.input_device = d.input_device.trim().chars().take(200).collect();
