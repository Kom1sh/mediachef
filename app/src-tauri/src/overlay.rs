...
pub fn show(app: &AppHandle) {
    ...
    // Позиция и размер — пока окно невидимо. macOS не даёт обычному окну
    // залезть на строку меню и прижимает его вниз, но к невидимому окну это
    // не применяется, а к моменту показа оно уже панель уровня 25.
    ...
    // Позицию ставим ещё раз: смена стиля окна на macOS может сдвинуть кадр.
    let _ = win.set_position(LogicalPosition::new(x, 0.0));
}
